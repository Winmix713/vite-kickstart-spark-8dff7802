...
describe('CMS Theme Integration Tests', () => {
  ...
  describe('Theme Selection Flow', () => {
    it('should complete a full theme selection workflow', () => {
      ...
      store.dispatch(setColorOverride({
        pageId,
        colorKey: 'primary',
        colorValue: '#FF0000',
      }));
  ...
  describe('Widget Variant Management', () => {
    it('should manage multiple widget variants on a page', () => {
      ...
      store.dispatch(setWidgetVariant({
        pageId,
      ...
      expect(selectWidgetVariant(state.cmsPage, pageId, 'widget-1')).toBe('compact');
      expect(selectWidgetVariant(state.cmsPage, pageId, 'widget-2')).toBe('minimal');
      expect(selectWidgetVariant(state.cmsPage, pageId, 'widget-3')).toBe('elevated');
    });
  });
  ...
  describe('Theme Variant CSS Variable Generation', () => {
    ...
    it('should have consistent CSS variable names across variants', () => {
      ...
      expect(defaultVarKeys.some(key => key.includes('--cms-color'))).toBe(true);
      expect(glassVarKeys.some(key => key.includes('--cms-color'))).toBe(true);
    });
  });
  ...
  describe('Multi-Page State Management', () => {
    it('should manage overrides for multiple pages independently', () => {
      ...
      store.dispatch(setPageThemeVariant({ pageId: page2, variant: 'emerald' }));
