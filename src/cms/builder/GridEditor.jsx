...
export const GridEditor = () => {
  ...
  const { layout, instancesById, selectedInstanceId } = useSelector(state => state.pageBuilder);
  ...
  const handleWidgetClick = useCallback((instanceId) => {
    dispatch(selectInstance(instanceId));
  }, [dispatch]);

  const handleRemoveWidget = useCallback((instanceId, e) => {
    e.stopPropagation();
    dispatch(removeInstance(instanceId));
  }, [dispatch]);

  const handleDuplicateWidget = useCallback((instanceId, e) => {
    e.stopPropagation();
    ...
    <div className="grid-editor h-full relative">
      ...
      {layout.length === 0 && (
        <div className="grid-placeholder flex flex-col items-center justify-center h-full text-gray-400">
          ...
          <p className="text-lg font-medium">Drop widgets here to get started</p>
          <p className="text-sm mt-1">Drag from the widget picker or click to add</p>
        </div>
      )}
    </div>
  );
