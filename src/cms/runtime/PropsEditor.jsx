...
const PropsEditor = ({ pageId, onSave }) => {
  ...
  const pageThemeMode = useSelector(state => selectPageThemeMode(state, pageId));
  const pageThemeVariant = useSelector(state => selectPageThemeVariant(state, pageId));
  const colorOverrides = useSelector(state => selectColorOverrides(state, pageId));
  ...
  const handleWidgetVariantChange = useCallback((widgetInstanceId, variant) => {
    dispatch(setWidgetVariant({ pageId, widgetInstanceId, variant }));
  }, [dispatch, pageId]);

  // Handle color override
  const handleColorChange = useCallback((colorKey, colorValue) => {
    dispatch(setColorOverride({ pageId, colorKey, colorValue }));
    setColorPicker(null);
  }, [dispatch, pageId]);

  // Handle color override removal
  const handleColorRemove = useCallback((colorKey) => {
            ...
            <div className="props-editor__color-grid">
              {Object.entries(colorOverrides || {}).map(([key, value]) => (
                <div key={key} className="props-editor__color-item">
                  <div className="props-editor__color-preview" style={{ backgroundColor: value }} />
                  ...
                  {widget.styleVariants && widget.styleVariants.length > 0 ? (
                    <div className="props-editor__variant-options">
                      {widget.styleVariants.map(variant => (
                        <label key={variant.slug} className="props-editor__variant-label">
                          <input
                            type="radio"
                            name={`widget-variant-${widget.id}`}
                            value={variant.slug}
                            onChange={(e) => handleWidgetVariantChange(widget.id, e.target.value)}
                    ...
                    <p className="props-editor__no-variants">No style variants available</p>
                  )}
                </div>
              ))}
            </div>
