...
export class AdminAPIService {
  ...
  async getUsers(options: AdminServiceOptions = {}) {
    ...
    if (useCache) {
      return adminConnectionManager.fetchWithCache(
        'user_profiles',
  ...
  async getModels(options: AdminServiceOptions = {}) {
    ...
    if (useCache) {
      return adminConnectionManager.fetchWithCache(
        'model_registry',
        (query) => query.select('*').order('registered_at', { ascending: false }),
        { ttl: cacheTTL }
      );
    }
  ...
  async getMatches(limit?: number, options: AdminServiceOptions = {}) {
    ...
    if (useCache) {
      return adminConnectionManager.fetchWithCache(
        ...
        { ttl: cacheTTL, cacheKey: `matches:${limit || 'all'}` }
      );
    }
    ...
    if (error) {
      logger.error('Failed to fetch matches', error, { limit }, 'AdminAPIService');
      return null;
    }
  ...
  async getJobs(options: AdminServiceOptions = {}) {
    const { useCache = true, cacheTTL = 30000 } = options;
  ...
  async updateUser(userId: string, updates: Record<string, unknown>) {
    try {
      ...
      logger.info('User updated successfully', { userId }, 'AdminAPIService');
      return { success: true, data };
    } catch (error) {
      logger.error('Failed to update user', error, { userId, updates }, 'AdminAPIService');
  ...
  async deleteUser(userId: string) {
    try {
      ...
      logger.info('User deleted successfully', { userId }, 'AdminAPIService');
      return { success: true };
    } catch (error) {
      logger.error('Failed to delete user', error, { userId }, 'AdminAPIService');
      return { success: false, error };
    }
  }
  ...
  async updateModel(modelId: string, updates: Record<string, unknown>) {
    try {
      ...
      logger.info('Model updated successfully', { modelId }, 'AdminAPIService');
      return { success: true, data };
    } catch (error) {
      logger.error('Failed to update model', error, { modelId, updates }, 'AdminAPIService');
      return { success: false, error };
    }
  }
