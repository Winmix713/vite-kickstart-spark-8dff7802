...
import { Area, AreaChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis, } from PageHeader from "@layout/PageHeader"; import { usePersistentState } from { winmixPredictionAccuracy, winmixUpcomingPredictions } from "../../data/predictions";
import classNames from 'classnames';

const ranges = ["Utolsó 7 nap", "Utolsó 30 nap"]; const Badge = ({ className, children }) => { const baseClasses = "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold transition-colors focus: outline-none focus, ring-2 focus:ring-ring focus:ring-offset-2"; return ( <span className={classNames(baseClasses, className)}> {children} </span> ); }; const AdminPredictions = () => { const[range, setRange] = usePersistentState("winmixpro-prediction-range", ranges[0]); return ( <>
<PageHeader title="Predikciók és pontosság" metaDescription="Modell- és crowd-pontosság, valamint piaci delta – teljesen mockolt idősorral." />
<div className="p-4 space-y-4">
<div className="flex flex-wrap gap-2 justify-end"> {ranges.map((item) => ( <button key={item} className={classNames("btn btn--sm", { "btn--primary": item === range, "btn--outline": item !== range, "bg-emerald-500 text-white hover: bg-emerald-600", item === range, "bg-black/40 text-white/70 hover: bg-black/30 border-transparent", item !== range })} onClick={() => setRange(item)} > {item} </button> ))} </div>
...
<div className="mt-4 grid gap-4"> {winmixUpcomingPredictions.map((item) => ( <div key={item.id} className="rounded-2xl border border-white/10 bg-black/40 p-4">
...
<div className="h-full rounded-full bg-emerald-400" style={{ width: '${item.confidence}%' }} />
</div>
</div>
<div>
<p className="text-xs uppercase text-white/50">Piac delta</p>
<p className={'text-sm font-semibold ${ item.marketDelta >= 0 ? "text-emerald-200" : "text-rose-200" }'} > {item.marketDelta >= 0 ? "+" : ""} {item.marketDelta}% </p>
</div>
</div>
</div> ))} </div>
</div>
