...
import { winmixPredictionAccuracy, winmixUpcomingPredictions } from "../../data/predictions";
import classNames from 'classnames';

const ranges = ["Utolsó 7 nap", "Utolsó 30 nap"];
...
const AdminPredictions = () => {
  const [range, setRange] = usePersistentState("winmixpro-prediction-range", ranges[0]);

  return (
    <>
      <PageHeader
        title="Predikciók és pontosság"
        metaDescription="Modell- és crowd-pontosság, valamint piaci delta – teljesen mockolt idősorral."
      />
          ...
          {ranges.map((item) => (
            <button
              ...
              className={classNames("btn btn--sm", {
                "btn--primary": item === range,
                "btn--outline": item !== range,
                "bg-emerald-500 text-white hover:bg-emerald-600": item === range,
                "bg-black/40 text-white/70 hover:bg-black/30 border-transparent": item !== range
              })}
